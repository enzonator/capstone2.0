...
@media (max-width: 991px) {
  ...
  /* Special handling for notification dropdown on mobile */
  nav.navbar.catshop-header #notifDropdownMenu {
...
@media (max-width: 576px) {
  ...
  /* Ensure dropdowns are visible and centered on small screens */
  nav.navbar.catshop-header .dropdown-menu {
  ...
  /* Make cart and notification icons more touch-friendly */
  nav.navbar.catshop-header .cart-link,
  nav.navbar.catshop-header .notif-link {
...
/* Desktop-specific positioning to keep right-aligned */
@media (min-width: 992px) {
  ...
  nav.navbar.catshop-header #userDropdownMenu {
    min-width: 10rem !important;
  }
}
  </style>
...
<!-- Custom Dropdown Script with Real-time Notifications -->
<script>
...
function fetchNotifications() {
  fetch('fetch-notifications.php')
    ...
    .then(data => {
      if (data.success) {
        updateNotificationDisplay(data.notifications, data.unreadCount);
      }
    })
    .catch(error => {
...
function updateNotificationDisplay(notifications, unreadCount) {
  ...
  // Build the notification list HTML
  let menuHTML = `
    <li class="px-3 py-2">
      <h6 class="mb-0" style="color: var(--accent-color);">
        <i class="bi bi-bell me-2"></i>Notifications
  ...
  if (notifications.length > 0) {
    notifications.forEach(notif => {
      ...
      menuHTML += `
        <li>
          <a class="dropdown-item notif-item ${unreadClass}" href="notification-view.php?id=${notif.id}">
...
document.addEventListener('DOMContentLoaded', function() {
  // Only fetch notifications if user is logged in (check if notif icon exists)
...
console.log('Custom dropdown system with real-time notifications loaded');
</script>
