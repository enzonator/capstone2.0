...
$adopter_id  = intval($_GET['adopter_id'] ?? 0);

if (!$cat_id || !$adopter_id) {
    die("Invalid request.");
}

// Fetch cat info and verify ownership
$catSql = "SELECT name, user_id FROM adoption_cats WHERE id = ?";
...
// Verify that the current user is the cat owner
if ($cat['user_id'] != $user_id) {
    die("You don't have permission to access this conversation.");
}
...
if ($markStmt) {
    $markStmt->bind_param("iii", $cat_id, $adopter_id, $user_id);
    $markStmt->execute();
    $markStmt->close();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && !empty(trim($_POST['message']))) {
    ...
    $stmt->bind_param("iiis", $cat_id, $user_id, $adopter_id, $message);
    $stmt->execute();
    $stmt->close();

    // Create notification for the adopter
    $senderUsername = $_SESSION['username'] ?? 'Pet Owner';
    $catName = $cat['name'] ?? 'a cat';
    $notificationMessage = "$senderUsername replied to your message about the adoption of $catName";
    ...
    $appIdStmt->bind_param("ii", $cat_id, $adopter_id);
    $appIdStmt->execute();
    ...
    if ($appIdResult->num_rows > 0) {
        ...
        error_log("Warning: Could not find approved application for cat_id: $cat_id, adopter_id: $adopter_id");
    }
    ...
    if ($application_id) {
        ...
        $notifStmt->bind_param("isii", $adopter_id, $notificationMessage, $cat_id, $application_id);
    } else {
        ...
        $notifSql = "INSERT INTO notifications (user_id, message, type, cat_id, is_read, created_at) 
                     VALUES (?, ?, 'adoption_message', ?, 0, NOW())";
        $notifStmt = $conn->prepare($notifSql);
        $notifStmt->bind_param("isi", $adopter_id, $notificationMessage, $cat_id);
    }
    ...
    header("Location: message-cat-owner.php?cat_id={$cat_id}&adopter_id={$adopter_id}");
    exit();
}
...
$msgStmt->bind_param("iiiii", $cat_id, $user_id, $adopter_id, $adopter_id, $user_id);
$msgStmt->execute();
    ...
    <div class="messages" id="messages">
        ...
        <?php else: ?>
            <div class="no-messages">
                ...
                <p>No messages yet. Start the conversation about <?= htmlspecialchars($cat['name']); ?>'s adoption!</p>
            </div>
        <?php endif; ?>
    </div>
...
function fetchMessages() {
    fetch("fetch-adoption-messages.php?cat_id=<?= $cat_id ?>&owner_id=<?= $adopter_id ?>")
        .then(res => res.json())
        .then(data => {
            renderMessages(data);
        })
